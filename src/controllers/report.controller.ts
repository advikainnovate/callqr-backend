...
export class ReportController {
  createBugReport = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const { description, severity } = req.body;
      const identity = req.identity;
      const userId = identity?.type === 'user' ? identity.userId : undefined; // Optional - allows anonymous reports
  ...
  getMyBugReports = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      ...
      const bugReports = await bugReportService.getUserBugReports(
        userId,
        limit
      );
  ...
  updateBugReportStatus = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const { reportId } = req.params;
      const { status } = req.body;

      const bugReport = await bugReportService.updateBugReportStatus(
        reportId,
        status
      );
  ...
  updateBugReportSeverity = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const { reportId } = req.params;
      const { severity } = req.body;

      const bugReport = await bugReportService.updateBugReportSeverity(
        reportId,
        severity
      );
      ...
      sendSuccessResponse(
        res,
        200,
        'Bug report severity updated successfully',
        {
          id: bugReport.id,
          severity: bugReport.severity,
        }
  ...
  getBugReportsBySeverity = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      ...
      sendSuccessResponse(
        res,
        200,
        `Bug reports with severity ${severity} retrieved successfully`,
        {
          reports: bugReports.map(report => ({
            id: report.id,
            userId: report.userId,
  ...
  getBugReportsByStatus = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      ...
      sendSuccessResponse(
        res,
        200,
        `Bug reports with status ${status} retrieved successfully`,
        {
          reports: bugReports.map(report => ({
            id: report.id,
            userId: report.userId,
