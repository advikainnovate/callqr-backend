...
export class SubscriptionController {
  createSubscription = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  getActiveSubscription = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user!.userId;
    const subscription = await subscriptionService.getActiveSubscription(userId);

    if (subscription) {
      sendSuccessResponse(res, 200, 'Active subscription retrieved successfully', {
        id: subscription.id,
        plan: subscription.plan,
        status: subscription.status,
        startedAt: subscription.startedAt,
  ...
  getUserSubscriptions = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user!.userId;
    const subscriptions = await subscriptionService.getUserSubscriptions(userId);

    sendSuccessResponse(res, 200, 'Subscription history retrieved successfully', {
  ...
  upgradePlan = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user!.userId;
  ...
  getUserPlan = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user!.userId;
